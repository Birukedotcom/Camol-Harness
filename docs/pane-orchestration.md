...
| `/vcs [OFFSET]`, `camol vcs inspect\|impact\|propose\|apply` | Candidate relationships, owner review and prospective rerun sets | Implemented for same-run candidates; GitHub reads use separate `observe`/`observation` commands; reviewed publication uses `propose-push`/`push`/`push-status`, never pane navigation; cross-run scope remains open |
